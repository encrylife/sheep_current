...
//                startActivityForResult(intent, 0);
    ...
    private void publish() {
        ...
        XHttpRequest xHttpRequest = new GongYingPublishRequest(1, nameet.getText().toString().trim(), priceet.getText().toString().trim(), leiMuId, typeid, timeet.getText().toString().trim(), detailet.getText().toString().trim(), linkman.getText().toString().trim(), linkphone.getText().toString().trim(), "", unit.getSelectedItem().toString(), "refresh");
        HttpHelper.request(xHttpRequest, PublishDo.class, new HttpHelper.Callback<PublishDo>() {
            ...
            public void onSuccess(PublishDo publishDo) {
                ...
                if (selectphotos != null && selectphotos.size() > 0) {
                    ...
                    Toast.makeText(SupplyPublish.this, "发布成功", Toast.LENGTH_LONG).show();
                    finish();
                }
            }
            ...
            public void onFailure(int errorCode, String msg) {
                ...
                Toast.makeText(SupplyPublish.this, msg, Toast.LENGTH_LONG).show();
                if (errorCode == -11) {
                    SupplyPublish.this. startActivityForResult(new Intent(SupplyPublish.this, Yhlogin.class),0);
                }
            }
        });
    }
...
//            Log.i("debug", datas.toString());
//            Map<String, String> files = new HashMap<String, String>();
//
//            try {
//                httpClientUtil.postRequest("http://www.51kuaibu.com/app/postSell.php?", datas);
//            } catch (Exception e) {
//                Toast.makeText(getApplicationContext(), R.string.server_connect_error, 0).show();
//                Log.i("debug", "=======");
...
//            typeet.setText(sb);
        ...
        } else if (requestCode == 0 && resultCode == 120) {
            linkman.setText(glApplication.getMwzuser().truename);
            linkphone.setText(glApplication.getMwzuser().mobile);
        }
    }
